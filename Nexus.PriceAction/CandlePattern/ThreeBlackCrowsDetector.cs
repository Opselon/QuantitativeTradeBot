...
{
    // Declares the ThreeBlackCrowsDetector class implementing the IPriceActionEngine contract. [Ref: Engine-Def]
    public class ThreeBlackCrowsDetector : IPriceActionEngine
    {
        // Thread-safely attaches a pattern storage dictionary to the processed context. [Ref: Dynamic-Attach]
        private static readonly ConditionalWeakTable<PriceActionContext, ConcurrentDictionary<DateTime, bool>> ThreeBlackCrowsCache = new();

        // Exposes a public helper to query the detected Three Black Crows state at any given timestamp. [Ref: Public-API]
        public static bool IsThreeBlackCrows(PriceActionContext context, DateTime timestamp)
        {
            // Tries to look up the attached pattern dictionary dynamically from the context. [Ref: Cache-Lookup]
            if (ThreeBlackCrowsCache.TryGetValue(context, out var dictionary))
            {
        ...
        // Detects and registers Three Black Crows patterns asynchronously in the pipeline. [Ref: Pipeline-Execute]
        public Task<PriceActionContext> AnalyzeAsync(PriceActionContext context, CancellationToken cancellationToken = default)
        {
            ...
            // Gets or attaches a thread-safe dictionary for Three Black Crows patterns. [Ref: Cache-Write]
            var crowsDict = ThreeBlackCrowsCache.GetOrCreateValue(context);
            ...
            crowsDict.TryAdd(context.RawCandles[0].Timestamp, false);

            // Sets false for the second candle's pattern state as it lacks enough historical bars. [Ref: Boundary-Set]
            crowsDict.TryAdd(context.RawCandles[1].Timestamp, false);
            ...
            for (int i = 2; i < context.RawCandles.Count; i++)
            {
                ...
                // Rule 1: All three candles must be strongly bearish. [Ref: Pattern-Rules]
                bool allBearish = prev2Analysis.IsBearish && prev1Analysis.IsBearish && currAnalysis.IsBearish;
                ...
                // Rule 3: Sequential descending close progression (making continuous lower closes). [Ref: Pattern-Rules]
                bool descendingCloses = currClose < prev1Close && prev1Close < prev2Close;

                // Rule 4: Candle 2 open must occur inside Candle 1's real body (Open is top, Close is bottom for bearish). [Ref: Pattern-Rules]
                bool secondOpenInFirstBody = prev1Open < prev2Open && prev1Open > prev2Close;

                // Rule 5: Candle 3 open must occur inside Candle 2's real body (Open is top, Close is bottom for bearish). [Ref: Pattern-Rules]
                ...
                bool isThreeCrows = allBearish && strongBodies && descendingCloses && secondOpenInFirstBody && thirdOpenInSecondBody;

                // Atomically registers the detection state in the attached dictionary. [Ref: State-Persist]
                crowsDict.AddOrUpdate(currCandle.Timestamp, isThreeCrows, (key, oldVal) => isThreeCrows);
            }
