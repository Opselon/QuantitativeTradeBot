    ...
    public class ExecutionCoordinator
    {
        ...
        public async Task<ExecutionResult> ProcessSignalAsync(TradeSignal signal, PipelineContext context, CancellationToken cancellationToken = default)
        {
            ...
            if (string.IsNullOrWhiteSpace(signal.SymbolName) || signal.Volume <= 0 || signal.Price <= 0)
            {
                ...
                _logger.LogWarning("[CorrID: {CorrelationId}] Signal validation failed: {Msg}", corrId, errMsg);
            ...
            try
            {
                ...
                _logger.LogInformation("[CorrID: {CorrelationId}] Dispatching order execution command to Gateway...", corrId);
            ...
            {
                _logger.LogError(ex, "[CorrID: {CorrelationId}] Unexpected error in Execution Pipeline.", corrId);
